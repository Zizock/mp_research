...
) |> select(Date, JGB_level, MB_level, BOJCA_level)
...
vix_list <- all_sheet |> 
  map(~ read_excel(file_name, sheet = .x) |> 
        mutate(Sheet = .x)
  )
...
# ==== pick one: ====
# can consider moving this part to the top
#qe_indicator = "JGB"
qe_indicator = "MB"
#qe_indicator = "BOJCA"
...
output_name = paste0("bvar_data_alllogdiff_", qe_indicator, "_", firm_size, ".xlsx")
...
get_sector_df <- function(sector, gdp_var, inflation_var, loan_var, qe_var, vix_var) {
  out <- list(
    ...
    get(paste0("cleaned_loan_", firm_size)) |> select(Date, !!sym(loan_var)) |> 
...
names(sector_data) <- paste0(firm_size, "_", sector_mapping$sector)

# 5, export to excel sheets
write_xlsx(sector_data, here("data", output_name))
